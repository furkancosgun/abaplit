...
CLASS ltcl_view_builder IMPLEMENTATION.
  ...
  METHOD test_ele_descend.
    ...
    cl_abap_unit_assert=>assert_equals(
      exp = '<Page><Button text="Click"/><Title text="T"/></Page>'
      act = lo_cut->stringify( ) ).
  ENDMETHOD.
  ...
  METHOD test_tag_sibling_and_a.
    ...
    cl_abap_unit_assert=>assert_equals(
      exp = '<VBox><Button text="Click"/><Text text="Hello"/><Text/></VBox>'
      act = lo_cut->stringify( ) ).
  ENDMETHOD.
  ...
  METHOD test_boolean_attr.
    ...
    cl_abap_unit_assert=>assert_equals(
      exp = '<Input visible="true" editable="false" value="X"/>'
      act = lo_cut->stringify( ) ).
  ENDMETHOD.
  ...
  METHOD test_stringify_from_root.
    ...
    cl_abap_unit_assert=>assert_equals(
      exp = '<mvc:View><Page><Content><Text text="deep"/></Content></Page></mvc:View>'

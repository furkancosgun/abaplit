...
class lcl_empty_filter definition final.
  public section.
    interfaces zif_abaplit_ajson_filter.
endclass.

class lcl_empty_filter implementation.
  method zif_abaplit_ajson_filter~keep_node.

    rv_keep = boolc(
      ( iv_visit = zif_abaplit_ajson_filter=>visit_type-value and
        is_node-type <> zif_abaplit_ajson_types=>node_type-number and is_node-value is not initial ) or " string & bool & null
      ( iv_visit = zif_abaplit_ajson_filter=>visit_type-value and
        is_node-type = zif_abaplit_ajson_types=>node_type-number and is_node-value <> '0' ) or " num
      ( iv_visit <> zif_abaplit_ajson_filter=>visit_type-value and is_node-children > 0 ) ). " array & object
...
class lcl_paths_filter implementation.
  ...
  method constructor.
    ...
    if boolc( iv_skip_paths is initial ) = boolc( it_skip_paths is initial ). " XOR
      zcx_abaplit_ajson_error=>raise( 'no filter path specified' ).
    endif.
...
class lcl_and_filter definition final.
  public section.
    ...
    methods constructor
      importing
        it_filters type zif_abaplit_ajson_filter=>ty_filter_tab
      raising
        zcx_abaplit_ajson_error.
  private section.
    data mt_filters type zif_abaplit_ajson_filter=>ty_filter_tab.
endclass.

CLASS zcl_abaplit_http_onprem DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
...
CLASS zcl_abaplit_http_onprem IMPLEMENTATION.
  METHOD constructor.
    IF server IS BOUND.
      TRY.
  ...
  METHOD zif_abaplit_http~get_path.
    ...
    IF result IS INITIAL.
      result = zif_abaplit_http~get_header( '~path' ).
    ENDIF.
    IF result IS INITIAL.
      result = zif_abaplit_http~get_header( '~path_info' ).
    ENDIF.
  ENDMETHOD.

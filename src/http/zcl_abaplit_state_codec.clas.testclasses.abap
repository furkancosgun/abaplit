...
CLASS ltcl_state_codec IMPLEMENTATION.
  METHOD test_serialize_uppercase.
    ...
    DATA(lv_json) = zcl_abaplit_state_codec=>serialize( lo_app ).

    TRY.
        DATA(lo_ajson) = zcl_abaplit_ajson=>parse( lv_json ).
        cl_abap_unit_assert=>assert_equals( exp = 'Demo'
                                            act = lo_ajson->get_string( '/MS_DATA/LABEL' ) ).
        cl_abap_unit_assert=>assert_equals( exp = 3
                                            act = lo_ajson->get_number( '/MS_DATA/SIZE' ) ).
        cl_abap_unit_assert=>assert_equals( exp = 'EDIT'
                                            act = lo_ajson->get_string( '/MV_MODE' ) ).
  ...
  METHOD test_hydrate_roundtrip.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 'Hydrated'
                                        act = lo_app->ms_data-label ).
    cl_abap_unit_assert=>assert_equals( exp = 9
                                        act = lo_app->ms_data-size ).

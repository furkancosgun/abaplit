CLASS zcl_abaplit_http_handler DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING http TYPE REF TO zif_abaplit_http.

  PRIVATE SECTION.
    CONSTANTS gc_content_json TYPE string VALUE 'application/json; charset=utf-8'.

    DATA mo_http TYPE REF TO zif_abaplit_http.
...
CLASS zcl_abaplit_http_handler IMPLEMENTATION.
  METHOD factory_onprem.
    DATA(lo_service) = NEW zcl_abaplit_http_onprem( server = server ).
    DATA(lo_handler) = NEW zcl_abaplit_http_handler( lo_service ).
    lo_handler->serve( ).
  ENDMETHOD.

  METHOD factory_cloud.
    DATA(lo_service) = NEW zcl_abaplit_http_cloud( request = request
                                                  response = response ).
    DATA(lo_handler) = NEW zcl_abaplit_http_handler( lo_service ).
    lo_handler->serve( ).
  ENDMETHOD.
  ...
  METHOD serve_roundtrip.
    TRY.
        DATA(ls_req) = zcl_abaplit_request_parser=>parse( mo_http->get_text( ) ).
        DATA(ls_res) = zcl_abaplit_app_runner=>run( ls_req ).
  ...
  METHOD respond_json.
    DATA(lv_json) = zcl_abaplit_response_builder=>build( res ).
  ...
  METHOD respond_info.
    DATA(lv_html) = zcl_abaplit_web_assets=>get_html( ).
    mo_http->set_header( name  = 'content-type'
                         value = 'text/html; charset=utf-8' ).
    mo_http->set_text( lv_html ).
    mo_http->set_status( 200 ).
    mo_http->set_compression( ).
  ENDMETHOD.
ENDCLASS.

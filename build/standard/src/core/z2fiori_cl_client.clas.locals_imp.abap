CLASS lcl_event_helper IMPLEMENTATION.
  ...
  METHOD event.
    ...
    FIELD-SYMBOLS <lv_arg> LIKE LINE OF t_arg.
    LOOP AT t_arg ASSIGNING <lv_arg>.
...
CLASS lcl_binding_resolver IMPLEMENTATION.
  ...
  METHOD resolve_oref.
    ...
    DATA lo_classdescr LIKE temp3.
    FIELD-SYMBOLS <fs_attr> LIKE LINE OF lo_classdescr->attributes.
    ...
    temp3 ?= cl_abap_typedescr=>describe_by_object_ref( io_data ).
  ...
  METHOD resolve_struct.
    ...
    FIELD-SYMBOLS <fs_comp> LIKE LINE OF io_desc->components.
          DATA lv_sub_path LIKE <fs_comp>-name.
